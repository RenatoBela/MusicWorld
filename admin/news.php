<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
		...
		$sql="INSERT INTO news (title, description, archive, tip) VALUES(?,?,?,?)";
            $stmt=mysqli_stmt_init($MySQL);

            if (mysqli_stmt_prepare($stmt, $sql)){

                mysqli_stmt_bind_param($stmt,'ssss',$_POST['title'] ,$_POST['description'],$_POST['archive'],$_POST['tip'] );
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
		$query  = "UPDATE news SET title='" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', description='" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', archive='" . $_POST['archive'] . "', tip='".$_POST['tip']."'";
	...
	#Add news 
		...
		<form action="" id="news_form" name="news_form" method="POST" enctype="multipart/form-data">
			...
			<label for="tip">Type:</label><br />
			<input type="radio" name="tip" value="music"/> Muzika &nbsp;&nbsp;
			<input type="radio" name="tip" value="event" checked/> Dogadaji
	...
	#Edit news
		...
		<form action="" id="news_form_edit" name="news_form_edit" method="POST" enctype="multipart/form-data">
			...
			<label for="tip">Type:</label><br />
            <input type="radio" name="tip" value="music"'; if($row['tip'] == 'music') { echo ' checked="checked"'; } echo ' /> Muzika &nbsp;&nbsp;
			<input type="radio" name="tip" value="event"'; if($row['tip'] == 'event') { echo ' checked="checked"'; } echo ' /> Dogadaji
